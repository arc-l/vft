...
max_plot_iteration = 10000  # Maximum number of training steps to report performance
...
parser.add_argument(
    ...
    type=str,
    nargs="+",
    help="path to session directories for which to plot performance",
)
...
colors = [
    [078.0 / 255.0, 121.0 / 255.0, 167.0 / 255.0],  # blue
...
for session_directory in session_directories:
    ...
    if len(model_list) > 0:
        ...
        print(
            "Error: no model weights saved, cannot determine whether session was trained in 'reactive' or 'reinforcement' mode."
        )
...
for session_idx in range(len(session_directories)):
    ...
    for step in range(max_iteration):
        ...
        grasp_attempt_ind_over_interval = grasp_attempt_ind_before_step[
            max(0, len(grasp_attempt_ind_before_step) - interval_size) : len(grasp_attempt_ind_before_step), 0
        ]
        ...
        if method == "reactive":
            grasp_success_over_interval = np.sum(reward_value_log[grasp_attempt_ind_over_interval] == 0) / float(
                min(interval_size, max(step, 1))
            )  # Class ID for successful grasping is 0 (reactive policy)
        elif method == "reinforcement":
            grasp_success_over_interval = np.sum(reward_value_log[grasp_attempt_ind_over_interval] >= 0.5) / float(
                min(interval_size, max(step, 1))
            )  # Reward value for successful grasping is anything larger than 0.5 (reinforcement policy)
        ...
        grasp_to_push_ratio_over_interval = float(
            np.sum(executed_action_log[max(0, step - interval_size) : step, 0] == 1)
        ) / float(min(interval_size, max(step, 1)))
        ...
        grasp_after_push_attempt_ind = (
            push_attempt_ind[np.argwhere(executed_action_log[push_attempt_ind[:, 0] + 1, 0] == 1), :] + 1
        )
        grasp_after_push_attempt_ind_before_step = grasp_after_push_attempt_ind[
            np.argwhere(grasp_after_push_attempt_ind[:, 0] < step)
        ]
        grasp_after_push_attempt_ind_over_interval = grasp_after_push_attempt_ind_before_step[
            max(0, len(grasp_after_push_attempt_ind_before_step) - interval_size) : len(
                grasp_after_push_attempt_ind_before_step
            ),
            0,
        ]
        ...
        if method == "reactive":
            grasp_after_push_success_over_interval = np.sum(
                reward_value_log[grasp_after_push_attempt_ind_over_interval] == 0
            ) / float(min(interval_size, max(step, 1)))
        elif method == "reinforcement":
            grasp_after_push_success_over_interval = np.sum(
                reward_value_log[grasp_after_push_attempt_ind_over_interval] >= 0.5
            ) / float(min(interval_size, max(step, 1)))
        # if step < interval_size:
    ...
    if len(session_directories) > 1 and session_idx + 1 == len(session_directories):
        ...
        plt.plot(
            range(0, max_iteration + 2000),
            push_then_grasp_success,
            dashes=[8, 7],
            color=color,
            linewidth=3,
            alpha=1,
            ...
            label="_nolegend_",
        )  # color='blue', dashes=[5,5], linewidth=2, dash_capstyle='butt')
