        ...
        self.head = nn.Sequential(
            OrderedDict(
                [
                    (
                        "grasp-head-conv000",
                        nn.Conv2d(1, 1, kernel_size=(GRIPPER_GRASP_SAFE_WIDTH_PIXEL, GRIPPER_GRASP_OUTER_DISTANCE_PIXEL), padding=(GRIPPER_GRASP_SAFE_WIDTH_PIXEL // 2, GRIPPER_GRASP_OUTER_DISTANCE_PIXEL // 2), bias=False),
                    ),
...
def resnet_fpn_net(
    ...
    backbone = resnet.__dict__[backbone_name](
        pretrained=False, norm_layer=norm_layer, input_channels=4
    )
    """
...
def resnet_fpn_backbone(
    backbone_name, pretrained, norm_layer=misc_nn_ops.FrozenBatchNorm2d, trainable_layers=3
):
